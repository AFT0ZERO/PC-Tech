...
- **A controller reaches past the database entirely:** `ProductController::fetchSpecs()` opens a raw PDO connection to `scraper/components.sqlite`, and `syncScraperConfig()` rewrites `scraper/config.json` on disk — external integration inside a presentation class.
